import os
def vivado_tcl_write(dir_name,string):

        rtl2syn = 'add_files -norecurse'+'{'+string+'}'+'\n'+ \
                  'update_compile_order - fileset sources_1'+'\n'+ \
                    'synth_design -part xc7k70t -top Subsystem'+'\n'+\
                    'write_verilog -force '+dir_name+'/'+'Subsystem_vivado.sv'
        tcl_rtl = dir_name + '/' + 'vivado_rtl2syn.tcl'
        f = open(tcl_rtl, "w")
        f.write(rtl2syn)
        f.close()
        filename =dir_name+ '/'+'Subsystem_vivado.sv'
        filename_testbench = dir_name + '/' +'Subsystem_tb.sv'
